...
def set_acs_format():
    ...
    def journal_bibitem(r):
        format = "%s %s %s, %s, %s." % (r['authors'], r['journal'], r['year'], r['volume'], r['pages'])
        return format
    JournalArticle.bibitem = journal_bibitem
    set('authors', JournalArticle, delim = ';', lastname = True)
    set('volume', JournalArticle, style = 'italic')
    set('pages', JournalArticle, lastpage = True) #no style modification
    set('year', JournalArticle, style = 'bold')
